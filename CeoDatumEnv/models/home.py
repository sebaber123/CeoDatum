...
class Home(object):
	...
	def create_table(cls, tableName , databaseName, dataType):
		...
		query = ("CREATE TABLE \""+databaseName+'-'+tableName + "\" (ID int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "+tableName+" "+dataType+" NOT NULL);")
	...
	@classmethod
	def create_fact_table(cls, databaseName, columns):
		

		con2 = get_db_visualization(databaseName)

		query = ("CREATE TABLE \""+databaseName+"\" (ID int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "+columns+");")
		cursor = con2.cursor(cursor_factory = psycopg2.extras.DictCursor)
		cursor.execute(query)
		con2.commit()
